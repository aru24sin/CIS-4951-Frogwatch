...
from firebase_admin import auth as admin_auth  # ✅ FIXED IMPORT
...
async def send_email(to_email: EmailStr = Query(...)):
    """
    Sends a simple welcome email and logs it to Firestore.
    """
    try:
...
# --- password-reset helper using security questions ---
...
async def verify_and_send_reset(req: ResetReq):
    """
    If email + 3 answers match what is in Firestore, email a Firebase reset link.
    ...
    try:
        # 1) Admin SDK: find user by email
        ...
        # 3) Build official Firebase reset link
